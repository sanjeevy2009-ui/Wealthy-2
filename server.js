...
const JWT_SECRET = process.env.JWT_SECRET || 'wealthy-dev-secret-change-in-production';
...
app.post('/api/auth/signup', (req, res) => {
  ...
  try {
    ...
    insN.run(uid, 'Explore AI Assistant', 'Get smart insights on your spending.', 'tip');
...
app.put('/api/auth/profile', auth, (req, res) => {
  ...
  db.prepare(`UPDATE users SET 
    ...
    WHERE id = ?`).run(name, card_number, card_holder, card_expiry, req.userId);
...
app.get('/api/health-score', auth, (req, res) => {
  ...
  else if (savingsRate >= 0) { factors.push({ label: 'Savings Rate', delta: 0, detail: `${savingsRate.toFixed(1)}% — needs improvement` }); }
...
app.post('/api/accounts', auth, (req, res) => {
  ...
  const r = db.prepare('INSERT INTO accounts (user_id, name, type, opening_balance, color) VALUES (?, ?, ?, ?, ?)').run(req.userId, name, type || 'bank', opening_balance || 0, color || 'green');
...
app.put('/api/accounts/:id', auth, (req, res) => {
  ...
  db.prepare(`UPDATE accounts SET 
    ...
    WHERE id = ? AND user_id = ?`).run(name, type, opening_balance, color, is_active, req.params.id, req.userId);
  res.json({ ok: true });
});
...
app.post('/api/accounts/transfer', auth, (req, res) => {
  ...
  const r = db.prepare('INSERT INTO transfers (user_id, from_account_id, to_account_id, amount, date, notes) VALUES (?, ?, ?, ?, ?, ?)').run(req.userId, from_account_id, to_account_id, amount, date || new Date().toISOString().split('T')[0], notes || '');
  res.json({ id: r.lastInsertRowid });
});
...
app.post('/api/transactions', auth, (req, res) => {
  ...
  const r = db.prepare('INSERT INTO transactions (user_id, account_id, merchant, category, method, amount, type, date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(req.userId, account_id || null, merchant, category, method, amount, type, date, notes || '');
  res.json({ id: r.lastInsertRowid });
});
...
app.put('/api/transactions/:id', auth, (req, res) => {
  ...
  db.prepare(`UPDATE transactions SET 
    ...
    WHERE id = ? AND user_id = ?`).run(merchant, category, method, amount, type, date, notes, account_id, req.params.id, req.userId);
  res.json({ ok: true });
});
...
app.post('/api/transactions/:id/duplicate', auth, (req, res) => {
  ...
  const r = db.prepare('INSERT INTO transactions (user_id, account_id, merchant, category, method, amount, type, date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(req.userId, t.account_id, t.merchant + ' (copy)', t.category, t.method, t.amount, t.type, new Date().toISOString().split('T')[0], t.notes);
  res.json({ id: r.lastInsertRowid });
});
...
app.get('/api/bills', auth, (req, res) => {
  res.json(db.prepare('SELECT * FROM bills WHERE user_id = ? ORDER BY id DESC').all(req.userId));
});

app.post('/api/bills', auth, (req, res) => {
  const { name, amount, due_date, status, icon, account_id, is_recurring, frequency } = req.body;
  const r = db.prepare('INSERT INTO bills (user_id, name, amount, due_date, status, icon, account_id, is_recurring, frequency) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(req.userId, name, amount, due_date, status || 'upcoming', icon || 'bill', account_id || null, is_recurring || 0, frequency || 'monthly');
  res.json({ id: r.lastInsertRowid });
});
...
app.get('/api/budgets', auth, (req, res) => {
  res.json(db.prepare('SELECT * FROM budgets WHERE user_id = ?').all(req.userId));
});
...
app.get('/api/goals', auth, (req, res) => {
  res.json(db.prepare('SELECT * FROM goals WHERE user_id = ?').all(req.userId));
});
...
app.get('/api/subscriptions', auth, (req, res) => {
  res.json(db.prepare('SELECT * FROM subscriptions WHERE user_id = ? ORDER BY id DESC').all(req.userId));
});

app.post('/api/subscriptions', auth, (req, res) => {
  const { name, amount, billing_cycle, next_billing, status, account_id, notes } = req.body;
  const r = db.prepare('INSERT INTO subscriptions (user_id, name, amount, billing_cycle, next_billing, status, account_id, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(req.userId, name, amount, billing_cycle || 'monthly', next_billing || null, status || 'active', account_id || null, notes || '');
  res.json({ id: r.lastInsertRowid });
});
...
app.get('/api/debts', auth, (req, res) => {
  res.json(db.prepare('SELECT * FROM debts WHERE user_id = ?').all(req.userId));
});

app.post('/api/debts', auth, (req, res) => {
  const { name, principal, remaining, interest_rate, min_payment, due_date, account_id } = req.body;
  const r = db.prepare('INSERT INTO debts (user_id, name, principal, remaining, interest_rate, min_payment, due_date, account_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(req.userId, name, principal, remaining || principal, interest_rate || 0, min_payment || 0, due_date || null, account_id || null);
  res.json({ id: r.lastInsertRowid });
});
...
app.put('/api/settings', auth, (req, res) => {
  ...
  db.prepare(`UPDATE settings SET 
    ...
    WHERE user_id = ?`).run(theme, accent, card_color, currency, upi_id, qr_code, req.userId);
  res.json(db.prepare('SELECT * FROM settings WHERE user_id = ?').get(req.userId));
});

app.put('/api/settings/currency', auth, (req, res) => {
  const { currency } = req.body;
  if (!currency) return res.status(400).json({ error: 'Currency required' });
  db.prepare('UPDATE settings SET currency = ? WHERE user_id = ?').run(currency, req.userId);
...
app.post('/api/transactions/import', auth, (req, res) => {
  ...
  if (!Array.isArray(transactions) || !transactions.length) {
    return res.status(400).json({ error: 'No transactions' });
  }
